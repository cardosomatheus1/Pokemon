...
> esses arquivos") e não pôde fazer.

## Conciliação com o código (25/09/2026)

| O cartão pressupõe | O código diz | Consequência |
|---|---|---|
| `run.iniciadaEm` | existe — `engine/run-avanco.mjs:75` | a hora da run sai dele |
| resolver em `engine/elenco-estagio.mjs` | `elencoDoEstagio(pack, biomaId, estagio)` | é aqui que a condição entra |
| chamadores | `avanco-estado.mjs:86` (a run) e `idle-escolha.mjs:221` (a sala, a prévia) | os dois precisam da condição; a prévia só do PERÍODO |
| — | uso INTERNO: a exclusão dos estágios anteriores e `estagiosDoBioma` | **invariante novo: a exclusão usa o elenco-BASE**, senão estágios vizinhos passam a se sobrepor |
| — | `engine/expedicao.mjs` tem um `elencoDoEstagio` HOMÔNIMO, de 4 argumentos e outro significado | **não mexer nele no 1.33**; a armadilha do nome vai para as lacunas |
| período do servidor | a run do Avanço é CLIENTE (save local); o servidor só guarda expedições | o 1.33 herda a autoridade do cliente; adiantar o relógio do aparelho burla — como já burla a stamina. Não se conserta dentro do 1.33 |
| fuso "UTC 06h/18h" como padrão de protótipo | **DEC-10 decidida pelo dono: Brasília para todos** — `relogioDoMundo` em `hora-do-dia.mjs` | período da run = `periodoEm(relogioDoMundo(run.iniciadaEm))` |
| períodos dia/noite | o 1.34 tem TRÊS: dia, tarde, noite | primeiro corte: só a NOITE muda preferência; dia e tarde são neutros |
| 6 mobs, 2 chefes | `COMUNS_POR_ESTAGIO 4`, `CHEFES_POR_ESTAGIO 2` (candidatos; 1 aparece na wave 10) | a troca é entre os 4 comuns |
